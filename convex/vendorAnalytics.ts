...
export const getVendorPerformance = query({
  ...
  handler: async (ctx) => {
    ...
    const winRate = totalQuotations > 0 ? (totalWonQuotations / totalQuotations) * 100 : 0;

    const totalRevenue = wonQuotations.reduce((sum, q) => sum + (q.price * q.quantity), 0);
    ...
    const averageDeliveryRating = ratings.length > 0
      ? ratings.reduce((sum, r) => sum + (r.deliveryRating || 0), 0) / ratings.length
      : 0;

    const averageCommunicationRating = ratings.length > 0
      ? ratings.reduce((sum, r) => sum + (r.communicationRating || 0), 0) / ratings.length
      : 0;

    const averageQualityRating = ratings.length > 0
    ...
    const validResponseTimes = rfqResponseTimes.filter((t) => t !== null) as number[];
  ...
  handler: async (ctx) => {
    ...
    const vendorStats = await Promise.all(
      allVendors.map(async (vendor) => {
        ...
        const winRate = quotations.length > 0 ? (wonQuotations.length / quotations.length) * 100 : 0;
    ...
    const marketAverageWinRate = activeVendors.length > 0
      ? activeVendors.reduce((sum, s) => sum + s.winRate, 0) / activeVendors.length
      : 0;

    const marketAverageRating = activeVendors.length > 0
      ? activeVendors.reduce((sum, s) => sum + s.avgRating, 0) / activeVendors.length
      : 0;
    ...
    const myWinRate = myQuotations.length > 0 ? (myWonQuotations.length / myQuotations.length) * 100 : 0;
...
export const getRecentPerformance = query({
  ...
  handler: async (ctx) => {
    ...
    return {
      ...
      revenueLast30Days: recentWonQuotations.reduce((sum, q) => sum + (q.price * q.quantity), 0),
    };
  },
});
